...
SILENCE_DURATION_THRESHOLD = 2.5 # Stop recording after 2.5 seconds of silence
WAIT_FOR_SPEECH_THRESHOLD = 4.0 # Wait 4 seconds for the user to start speaking

def setup_vad():
    vad = webrtcvad.Vad()
    vad.set_mode(2) # 0 to 3, 3 is most aggressive in filtering out non-speech
    return vad
        ...
        while True:
            ...
            if state == "WAKEWORD":
                ...
                if state_queue:
                    ...
                    state_queue.put({"type": "RMS", "value": float(rms)})
                ...
                if silence_frames > current_threshold_frames:
